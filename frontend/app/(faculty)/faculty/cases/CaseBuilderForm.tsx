...
export function CaseBuilderForm({ caseId }: { caseId?: string }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("cases")
      .select(
        "id, title, difficulty, est_minutes, stage, tags, age, sex, ecog, chief_complaint, medical_history, imaging, pathology, candidate_drugs, toxicity_concerns, clinical_pearl, objective_titles",
      )
      .eq("id", caseId)
      .maybeSingle<CaseRow>()
      .then(({ data }) => {
        ...
        setEstMinutes(data.est_minutes);
        setAge(data.age);
        setSex(data.sex);
        setEcog(data.ecog);
        setChiefComplaint(data.chief_complaint);
        setMedicalHistory((data.medical_history ?? []).join(", "));
        setImagingStudy(data.imaging?.[0]?.study ?? "");
        setImagingFinding(data.imaging?.[0]?.finding ?? "");
        setDiagnosis(data.pathology?.diagnosis ?? "");
        setMarkers((data.pathology?.markers ?? []).map((m) => `${m.name}:${m.value}`).join(", "));
        setGenomicProfile((data.pathology?.genomicProfile ?? []).join(", "));
        setCandidateDrugs((data.candidate_drugs ?? []).map((d) => d.name).join(", "));
        setToxicityConcerns((data.toxicity_concerns ?? []).join(", "));
        setClinicalPearl(data.clinical_pearl ?? "");
        setQuestions((data.objective_titles ?? []).join(", "));
      });
  }, [caseId]);
  ...
  async function save() {
    ...
    const supabase = createClient();
    const id = caseId ?? `faculty-${crypto.randomUUID()}`;
  ...
  if (loading) return <Shell breadcrumb="Case Builder"><div /></Shell>;
  if (notFound) {
    return (
      <Shell breadcrumb="Case Builder">
        <div className="mx-auto max-w-3xl px-6 py-8">
          <p className="text-[13px] text-muted-foreground">Case not found, or you don't own it.</p>
        </div>
      </Shell>
    );
  }
      ...
      <div className="mx-auto max-w-3xl px-6 py-8">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="font-heading text-[22px] font-bold tracking-tight text-foreground">
              {editing ? "Edit Case" : "Faculty Case Builder"}
            </h1>
            <p className="text-[13px] text-muted-foreground">
              {editing
                ? "Update this case — changes apply immediately for anyone in your institution."
        ...
        <div className="mt-6 flex items-center gap-3">
          <Button onClick={save} disabled={!isValid || saving} className="bg-navy text-white hover:bg-navy/90 disabled:opacity-50">
            {saving ? "Saving…" : editing ? "Save Changes" : "Publish Case to Institution"}
          </Button>
          {saved && (
            <span className="flex items-center gap-1.5 text-[12.5px] font-medium text-teal-deep">
              <CheckCircle2 size={15} /> Saved — opening case…
            </span>
          )}
        </div>
      </div>

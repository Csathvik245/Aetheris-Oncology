...
export default function ExamResultsPage({ params }: { params: Promise<{ examId: string; attemptId: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: qs } = await supabase
        .from("exam_questions")
        .select("id, order_index, stem, choices, correct_choice, explanation, citation")
        .eq("exam_id", examId)
        .order("order_index", { ascending: true })
        .returns<QuestionWithAnswer[]>();
      const { data: attempt } = await supabase.from("exam_attempts").select("answers, score").eq("id", attemptId).single();
      ...
      if (qs && qs.length > 0) {
        const correct = qs.filter((q) => attemptAnswers[q.id] === q.correct_choice).length;

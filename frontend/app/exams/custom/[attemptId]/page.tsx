...
export default function CustomExamTakePage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: attempt } = await supabase
        .from("exam_attempts")
        .select("custom_title, custom_question_ids, time_limit_minutes, started_at")
        .eq("id", attemptId)
        .single();
      if (!attempt) return;

      setExamTitle(attempt.custom_title ?? "Custom Exam");
      setTimeLimitMinutes(attempt.time_limit_minutes ?? 20);
      setSecondsLeft((attempt.time_limit_minutes ?? 20) * 60);
      startedAtRef.current = new Date(attempt.started_at).getTime();

      const ids: string[] = attempt.custom_question_ids ?? [];
      // No-hints: only stem/choices selected — correct_choice/explanation never fetched during the attempt.
      ...
      // Preserve the randomized order the attempt was built with, not DB order.
  ...
  async function submit() {
    ...
    await supabase
      ...
      .eq("id", attemptId);
    router.push(`/exams/custom/${attemptId}/results`);
  }
          ...
          <div className="flex gap-1">
            {questions.map((qq, i) => (
              <button
                ...
                className={`h-2 w-2 rounded-full ${i === current ? "bg-navy" : answers[qq.id] ? "bg-teal" : "bg-border"}`}
              />
            ))}
          </div>
